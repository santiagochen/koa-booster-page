...
const trimmedPathname=stripPrefix(pathname,decodeURIComponent("/koa-booster-page"))// Remove any hashfragment
...
if(this.staticQueryDb[staticQueryHash]){const jsonPayload=this.staticQueryDb[staticQueryHash];return{staticQueryHash,jsonPayload};}return this.memoizedGet(`${"/koa-booster-page"}/page-data/sq/d/${staticQueryHash}.json`).then(req=>{const jsonPayload=JSON.parse(req.responseText);return{staticQueryHash,jsonPayload};}).catch(()=>{throw new Error(`We couldn't load "${"/koa-booster-page"}/page-data/sq/d/${staticQueryHash}.json"`);});})).then(staticQueryResults=>{const staticQueryResultsMap={};staticQueryResults.forEach(({staticQueryHash,jsonPayload})=>{staticQueryResultsMap[staticQueryHash]=jsonPayload;this.staticQueryDb[staticQueryHash]=jsonPayload;});return staticQueryResultsMap;});return Promise.all([componentChunkPromise,staticQueryBatchPromise]).then(([pageResources,staticQueryResults])=>{let payload;if(pageResources){payload=_objectSpread(_objectSpread({},pageResources),{},{staticQueryResults});finalResult.payload=payload;emitter.emit(`onPostLoadPageResources`,{page:payload,pageResources:payload});}this.pageDb.set(pagePath,finalResult);if(finalResult.error){return{error:finalResult.error,status:finalResult.status};}return payload;})// when static-query fail to load we throw a better error
...
const config={gatsby:{pathPrefix:'/koa-booster-page',siteUrl:'https://santiagochen.github.io/koa-booster-page/',gaTrackingId:null,trailingSlash:false},header:{logo:'https://graphql-engine-cdn.hasura.io/learn-hasura/assets/homepage/brand.svg',logoLink:'https://santiagochen.github.io/koa-booster-page/',title:"<a href='https://hasura.io/learn/'><img class='img-responsive' src='https://graphql-engine-cdn.hasura.io/learn-hasura/assets/homepage/learn-logo.svg' alt='Learn logo' /></a>",githubUrl:'https://github.com/hasura/gatsby-gitbook-boilerplate',helpUrl:'',tweetText:'',social:`<li>
...
manifest:{name:'Gatsby Gitbook Starter',short_name:'GitbookStarter',start_url:'https://santiagochen.github.io/koa-booster-page/',background_color:'#6b37bf',theme_color:'#6b37bf',display:'standalone',crossOrigin:'use-credentials',icons:[{src:'src/pwa-512.png',sizes:`512x512`,type:`image/png`}]}}};module.exports=config;
...
/* harmony default export */ const components_theme = ({fonts:{mono:'"SF Mono", "Roboto Mono", Menlo, monospace'}});
...
"use strict";
module.exports = JSON.parse('{"polyfill":["/polyfill-94f96524d21b228bf347.js"],"app":["/app-06021d4a58516f368eb5.js"],"component---src-templates-docs-js":[]}');
